...
class MnistBags(data_utils.Dataset):
        ...
        if self.train:
            ...
            while valid_bags_counter < self.num_bag:
                ...
                if (self.target_number in labels_in_bag) and (label_of_last_bag == 0):
                    ### ---- 
                    ins_label_list.append(torch.zeros(len(labels_in_bag)))
                    ...
                    index_list = torch.tensor([x.numpy()[0] for x in index_list])
                    labels_in_bag = labels[index_list]

                    ###---
                    ins_label_list.append(labels[index_list])
            ...
            while valid_bags_counter < self.num_bag:
                ...
                if (self.target_number in labels_in_bag) and (label_of_last_bag == 0):
                    ...
                    index_list = torch.tensor([x.numpy()[0] for x in index_list])
                    labels_in_bag = labels[index_list]
                    # print("ins labels: ", labels[index_list])
                    ####---
                    ins_label_list.append(labels[index_list])
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            ###----
            # ins_label = self.train_ins_label_list[index]
        else:
            bag = self.test_bags_list[index]
            label = [max(self.test_labels_list[index]), self.test_labels_list[index]]
            ###---- 
            # ins_label = self.test_ins_label_list[index]
